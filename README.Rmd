...
The goal of state is to provide an alternative to `shiny::reactiveValues()` that includes type safety functionality. Create a type safe reactivaValues with `state_manager()` and initialize values with `state()`, where the first argument is the initial value and the second one is the expected S7 class. 
